...
func (h Hand) IsFullHouse() bool {
	...
	return (hasPair && hasThree) || h.NonJokerPairCount() == 2 && h.values[0] == 1
...
func (h Hand) IsThreeOfAKind() bool {
	...
	pairCount := h.NonJokerPairCount()
	if pairCount == 0 {
		return false
	}
	if pairCount == 1 {
		return h.values[0] == 1 || h.values[0] == 2
	}
	return false
}
func (h Hand) NonJokerPairCount() int {
	pairCount := 0
	for _, value := range h.values {
		if value == 2 {
			pairCount += 1
